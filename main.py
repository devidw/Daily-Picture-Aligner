...
if len(files) > 1:
    raise Exception(
        "Ensure there's only one base image, and that you deleted the initial text file."
    )
...
for file in tqdm(fileListSorted):
    ...
    file = (
        file.name
    )  # Pathlib returns it as a pathlib.WindowsPath instead of a string, and it returns the parent folder like this: parentfolder/file.jpg, so we need to convert it back into a string for the logic ahead using file.name, just the file name as string
    if fileSuffix + file + fileAffix in os.listdir(
        OutputPath
    ):  # if our file has already been aligned, do nothing.
        pass
    else:
        ...
        if (
            file.lower().endswith("g")
            and os.path.getsize(DailyPhotoPath + file) > 0
            and file != "1871.jpg"
        ):
            ...
            if success:
                cv.imwrite(
                    OutputPath + str(count) + fileSuffix + file + fileAffix,
