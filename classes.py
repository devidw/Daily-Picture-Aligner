...
class Image:
    def scaleDownImage(self):
        ...
        if self.Width > self.Height and (
            self.Height >= 3024 or self.Width >= 4032
        ):  # for some reason, if an image is too large, there seems to be some landmakr detection issues, therefore, we scale the image down by 2

            self.Height, self.Width = int(self.cvimage.shape[0] / 2), int(
                self.cvimage.shape[1] / 2
            )
            self.Dimensions = (
                self.Width,
                self.Height,
            )  # divide by 2 here, and not above, for some reason, this makes sure it is a INT and not an float!
            Matrix = cv.getRotationMatrix2D(
                (0, 0), 0, 0.5
            )  # leave it at (0,0) it seems to work better for 1 base image alignments. idk why YET
            self.cvimage = cv.warpAffine(
                self.cvimage, Matrix, self.Dimensions
    ...
    def __init__(self, libfile, CorrespondingBaseImage=None):
        ...
        self.CorrespondingBaseImage = (
            CorrespondingBaseImage if CorrespondingBaseImage is not None else None
        )  # straight from stack overflow
        try:
            self.LeftEyex, self.LeftEyey = (
                self.LeftEyeImageCoordinates[0],
                self.LeftEyeImageCoordinates[1],
            )
            self.RightEyex, self.RightEyey = (
                self.RightEyeImageCoordinates[0],
                self.RightEyeImageCoordinates[1],
            )
    ...
    def getImageCoordinates(
        self, targetlandmark
    ):  # converted into a streamlined function for accesiblity. can't speel. spell.
        try:
            with mp_face_mesh.FaceMesh(
                ...
                ):  # if there are no face landmarks detected it will ignore.
                    return None
                ...
                for face_landmarks in results.multi_face_landmarks:
                    for id, landmark in enumerate(face_landmarks.landmark):
                        if (
                            id == targetlandmark
                        ):  # this is the point I'm targeting: 468 is the left eye, 473 for the right looks great.
                            self.currentImageCoordinates = [
    ...
    def refreshEyeCoordinates(
        self,
    ):  # this is needed because after the transformations, the eye locations change! #updated so that it does the transformations with basic math. saves so much computational time.
        try:
            ...
            self.LeftEyex, self.LeftEyey = (
                self.LeftEyex * self.scaleFactor,
                self.LeftEyey * self.scaleFactor,
            )
            self.RightEyex, self.RightEyey = (
                self.RightEyex * self.scaleFactor,
                self.RightEyey * self.scaleFactor,
            )
            self.Xdifference = self.RightEyex - self.LeftEyex
        except Exception as error:
            pass  # handling errors within the alignImagetoBaseImage class

    def scaleAroundPoint(
        self, BaseImage
    ):  # this function scales the image by a calculated scalefactor to remove and differences in camera distance.
        try:
            self.scaleFactor = (
                BaseImage.Xdifference / self.Xdifference
            )  # a very simply formula i came up with, wasn't my first iteration, but it works now. I'm saying that like it's complex math, its literally a fraction ratio
            # x-y coord, rotation angle, scaling factor
            # scale from the center of img
            Matrix = cv.getRotationMatrix2D(
                (0, 0), 0, self.scaleFactor
            )  # I was looking for a way to scale around an image for so long, it was so simple. scaling from the corner
            self.cvimage = cv.warpAffine(
                self.cvimage, Matrix, self.Dimensions
            )  # multiplying by 2 to ensure the entire image stays in frame, then on the rotation (the final transfomration) we scale it back down to normal to ensure none of hte image gets cut off!
    ...
    def rotateImage(
        self, BaseImage
    ):  # this function rotates the image so that the slope of the eyes will align with the slope of the base image, if that makes sense. if it doesn't it just makes it better trust me.
        eyePoint = (
            self.LeftEyex,
            self.LeftEyey,
        )  # consider testing with Baseimage eye coordinates
        angle = np.rad2deg(
            np.arctan((self.RightEyey - BaseImage.RightEyey) / (self.Xdifference))
        )  # tangent formula right triangle.
        rotationalMatrix = cv.getRotationMatrix2D(eyePoint, angle, 1.0)
        self.cvimage = cv.warpAffine(
            self.cvimage, rotationalMatrix, self.Dimensions
        )  # we use the base image's width and height in case there are different sizes, so for base image, use your smallest camera resolution photo. i took some with webcam and my phone for example, if i use my webcame image as base image, my phone will be properly scaled down.
    ...
    def alignImagetoBaseImage(
        ...
        try:
            ...
            self.translate(
                movex, movey
            )  # consider doing another final translation? cuz translate shoould come last after scaling the image back down but this is just experimental.
            angle = self.rotateImage(
                BaseImage
            )  # this MUST come last. idk why, but try flipping translate and rotate and see how wonky it gets.
            if (
                self.Dimensions != BaseImage.Dimensions
            ):  # this is a check to see if we are have only 1 baseimage, this is the only time this wouldn't be equal if we have 1 base image for differing resolutioned images. it saves us throwing in an extra argument like "1baseimage" boolean, etc.
                self.cvimage = self.cvimage[
                    0 : BaseImage.Height, 0 : BaseImage.Width
...
class BaseImage(Image):
    def __init__(
        self, libfile
    ):  # unsure about the use and need for super. consult old spaceship game with classes. #commendting out the __init__ somehow allows the script to still 'work'
    ...
    def reset(
        self,
    ):  # basically calling the __init__ again, similar to making a new instance. this is just for the alignment info function to reset stats.
    ...
    def getAlignmentInfo(
        self, UltimateBaseImage
    ):  # eyePoint = (BaseImage.LeftEyex,BaseImage.LeftEyey)
        original = (
            self.cvimage
        ...
        self.UltimateBaseImageHeight, self.UltimateBaseImageWidth = (
            UltimateBaseImage.cvimage.shape[:2]
        )
        self.UltimateDimensions = (
            self.UltimateBaseImageWidth,
            self.UltimateBaseImageHeight,
        )
        ...
        Matrix = cv.getRotationMatrix2D(
            (0, 0), 0, self.UltimatescaleFactor
        )  # I was looking for a way to scale around an image for so long, it was so simple.
        self.cvimage = cv.warpAffine(
            self.cvimage, Matrix, UltimateBaseImage.Dimensions
        ...
        self.UltimateAngle = np.rad2deg(
            np.arctan(
                (self.RightEyey - UltimateBaseImage.RightEyey) / (self.ExactXdifference)
            )
        )  # tangent formula right triangle.
